...
const createComponent = (Component, config) => {
  const { Component, rules, variables, shorthand, getDefaultElement } = config
  const StyledComponent = rules ? connect(rules)(Component) : Component

  const UIComponent = props => (
    <FelaTheme
      render={(siteVariables) => {
        const mergedVariables = variables ? { ...variables(siteVariables), ...props.variables } : props.variables
